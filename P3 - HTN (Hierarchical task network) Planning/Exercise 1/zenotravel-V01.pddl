...
(:task transport-person
    ...
    ; La persona ya se encuentra en la ciudad destino
    (:method Case1 ; si la persona est� en la ciudad no se hace nada
	    :precondition (at ?p ?c)
	    :tasks ()
    )
	 
    ; Persona no esta en la ciudad destino pero puede ir porque el avion se encuentra en su misma ciudad
    ...
    ; Persona no puede ir a ciudad destino porque el avion no se encuentra en su ubicacion
    (:method Case3
      :precondition (and    (at ?p - person ?c1 - city)
      ...
      :tasks (  
                (fly ?a ?c2 ?c1)            ; El avion vuela hasta la persona
                (board ?p ?a ?c1)           ; La persona embarca en el avion
                (mover-avion ?a ?c1 ?c)     ; El avion vuela hasta la ciudad destino
                (debark ?p ?a ?c ))          ; La persona desembarca en la ciudad destino      
    )
